...
def _captured_launch(launcher: Path, *arguments: str) -> str:
    if not launcher.exists():
        raise AssertionError(f"launcher does not exist: {launcher}")
...
class WorkflowTests(unittest.TestCase):
    ...
    def test_phase1_launcher_requires_explicit_operation(self) -> None:
        phase1 = Path(__file__).parents[3] / "examples/balalaika/cosyvoice3_lora/run_phase1.sh"
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            fake_accelerate = root / "accelerate"
            fake_accelerate.write_text("#!/usr/bin/env bash\nexit 0\n", encoding="utf-8")
            fake_accelerate.chmod(0o755)
    ...
    def test_phase1_launcher_forwards_memorize_and_train_to_distinct_commands(self) -> None:
        phase1 = Path(__file__).parents[3] / "examples/balalaika/cosyvoice3_lora/run_phase1.sh"
        self.assertIn("phase1-memorize", _captured_launch(phase1, "--memorize").split())
        self.assertIn("phase1-train", _captured_launch(phase1, "--train").split())

    def test_prepare_cache_launcher_uses_eight_rank_cache_command(self) -> None:
        prepare_cache = Path(__file__).parents[3] / "examples/balalaika/cosyvoice3_lora/prepare_cache.sh"
        arguments = _captured_launch(prepare_cache)
        self.assertIn("--num_processes 8", arguments)
        self.assertIn("prepare-cache", arguments)
        source = prepare_cache.read_text(encoding="utf-8")
        for forbidden in ("torchrun", "WANDB_API_KEY", "HF_TOKEN", "huggingface_hub"):
    ...
    def test_recipe_has_exactly_three_accelerate_launchers(self) -> None:
        ...
        self.assertEqual(
            ...
            ["prepare_cache.sh", "run_phase1.sh", "run_phase2.sh"],
        )
        commands = {
            "prepare_cache.sh": ("prepare-cache",),
            "run_phase1.sh": ("phase1-memorize", "phase1-train"),
            "run_phase2.sh": ("phase2",),
        }
        for path in launchers:
            ...
            for command in commands[path.name]:
    ...
    def test_launchers_export_repository_and_matcha_on_pythonpath(self) -> None:
        ...
        for launcher in sorted(recipe.glob("*.sh")):
            with self.subTest(launcher=launcher.name), tempfile.TemporaryDirectory() as directory:
                ...
                subprocess.run(
                    ["bash", str(launcher), *launcher_arguments[launcher.name]],
                    check=True,
                    env=environment,
                )
