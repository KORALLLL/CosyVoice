...
- Root cause: The production launchers exported only the repository root, while CosyVoice3 flow construction imports `matcha` from the tracked `third_party/Matcha-TTS` submodule. The submodule was also uninitialized in this worktree.
- Changed: Both launchers now prepend the repository and pinned Matcha-TTS checkout to `PYTHONPATH`; the setup guide explicitly initializes that exact submodule. A behavioral shell test executes both launchers with a probe Python binary and verifies the resulting path order.
- Environment: Installed the repository-pinned pilot dependencies missing from the active CUDA environment and aligned NumPy 1.26.4 with SciPy 1.14.1. Direct construction of the frozen local `CosyVoice3` checkpoint now succeeds on GPU 0 and reports 24 kHz output.
- Real gate: Published the three-pair pilot and stopped with the intentional review-required exit before approval. The inner pilot manifest SHA-256 is `1a25919e2285ac90f0601ebc0e1c87668c728f9e4ab78b8db529f3030b06c5c3`.
...
- The original diagnostic accepted its trainable-parameter audit and finite-loss evidence. Final review found that its `cache_manifest_sha256=81966b2953c878da10300370fbc1790179eef64558d64ef8fb3e09f3f9af38d4` was a derived cache-content digest rather than the SHA-256 of `memorization_cache/manifest.json`; that historical cache field is superseded and a fresh authorized smoke run is required for hardened evidence. Its `base_llm_sha256=69f43bd545131c30e98947fb360ea8b4dc9916d8e83dded7757c7ea4f5a24970` remains the recorded base checksum. The official workflow-stage set remained exactly `pilot_ready.json`, `preflight_complete.json`, and `tokenizer_qualified.json`.
...
- Final regenerated evidence: the prior isolated directory was recoverably archived as `three_gpu_smoke.superseded-20260803-final`; one fresh three-GPU diagnostic then exited 0 with attempt ID `5029e7a442337dc69719c33b85aa37bf`.
- The fresh manifest records `world_size=3`, exactly two steps, and three finite losses per step: `[6.298747, 5.314507, 5.807076]` and `[4.633666, 5.399097, 4.654178]`. Its validated audit is bound to the current outer cache manifest SHA-256 `c0e2293f35580c3b1326a056c1baa3272735382a467e240e841b35c434719f1d`; its base LLM SHA-256 is `69f43bd545131c30e98947fb360ea8b4dc9916d8e83dded7757c7ea4f5a24970`.
- The official stage set remained exactly `pilot_ready.json`, `preflight_complete.json`, and `tokenizer_qualified.json`; the permitted GPUs were idle before launch and returned to 0 MiB / 0% afterward. No official phase ran.

## 2026-08-04 - Separated Phase-1 operational handoff

- Decision: Phase 1 is now three authenticated operations: `run_phase1.sh --memorize` performs pilot approval plus the four-sample memorization gate; `prepare_cache.sh` performs full speech-token cache extraction only; and `run_phase1.sh --train` requires both sealed prerequisites, then runs capacity smoke, baseline validation, and Phase-1 SFT.
- Decision: Capacity smoke remains inside the training operation because it is a bounded eight-GPU forward/backward probe that selects the largest safe token budget while reserving two GiB of headroom; it performs no corpus cache construction and is immediately followed by the baseline validation/training path.
- Validation: The complete Balalaika unittest discovery suite and capture-only launcher probes passed. The probes replace `accelerate` with a stub and confirm separate `phase1-memorize`, `prepare-cache`, and `phase1-train` commands, exact eight-process BF16 options, and repository-plus-Matcha `PYTHONPATH` ordering. Shell syntax, Python compilation, and whitespace checks also pass.
- Safety: This handoff ran no real tokenization, cache build, memorization, W&B, validation generation, training, or GPU workload. Production still starts with the approved pilot checksum and the memorization operation.
